...
def getparmas():  # true   4 API
    ...
    product_id = postValues.get('product_id', "")
    sale_id = postValues.get('sale_id', "")  # 上述两者二选一
    ...
    # # local test
    # from transwithgpt.v5with4.preparedataprocess import allTextList as souLangText  # 一个测试数据
    # product_id = '1047068'
    # # product_id = ''
    # # sale_id = '1004592911'
    # sale_id = ''
    ...
    souL = lang2line_name[souL][0]
    tarL = lang2line_name[line_code][0]
    _ = lang2line_name[line_code][1]
    product_name, ad_slogans, sale_name, product_name_ali, paramsFinal = '', '', '', '', {}
    # todo 通过传入的 pid or sid 和 line_code 获取所有参数以构建参数 ana check params
    res = part1toparams(product_id=product_id, sale_id=sale_id, line_code=line_code)
    if len(res) == 5:
        product_name, sale_name, _, _, product_name_ali = res
    if len(res) == 7:
        product_name, sale_name, ad_slogans, _, _, _, product_name_ali = res
    product_name = product_name.split('-')[1] if '-' in product_name else product_name
    if not tarL or not line_code or not souLangText or not (product_id and sale_id):
        logging.info(f'请求异常，请检查如下参数是否为空：\n'
                     f'1.tarL:{tarL}；'
                     f'2.line_cde:{line_code}；'
                     f'3.souLangText:{souLangText}；'
                     f'4.product_id:{product_id} 和 sale_id:{sale_id} 是否均为空。')
        return '0'
    if ad_slogans:
        paramsFinal = {
            'taskID': str(int(time.time())),  # 10数时间戳，该参数无需传入
            'taskName': 'transPicture',
            'product_id': product_id,
            'sale_id': sale_id,
            'souL': souL,  # 可以不传，默认CN
            'tarL': tarL,  # 必须要传；具体说明在：lang2line_name
            'souLangText': souLangText,
            'toDataPart': {
                "商品类别": product_name,
                f'{tarL}商品标题': sale_name,
                "中文商品标题": product_name_ali,
                f"{tarL}广告语": ad_slogans
            }
        }
    else:
        paramsFinal = {
    ...
    logging.info(f'【开始任务】本次任务完整参数信息如下：\n{paramsFinal}\n'
                 f'==============================================================================')
    ...
    try:
        ...
        res_part1 = eval(requests.post(
            url=gptURLFast, json={'message': [{'content': tp1, 'role': 'user'}], 'role': 'taskTransPicture'}).text)
    ...
    try:
        ...
        res_part2 = eval(requests.post(
            url=gptURLFast, json={'message': [{'content': tp2, 'role': 'user'}], 'role': 'taskTransPicture'}).text)
    ...
    # if len(souLangText) == 1:
    #     splitSeqList = [souLangText]
    # if 1 < len(souLangText) <= 6:
    #     splitSeqList = [souLangText[i:i + int(len(souLangText) / 2)] for i in
    #                     range(0, len(souLangText), int(len(souLangText) / 2))]
    # if 6 < len(souLangText) <= 10:
    #     splitSeqList = [souLangText[i:i + int(len(souLangText) / 3)] for i in
    #                     range(0, len(souLangText), int(len(souLangText) / 3))]
    # if len(souLangText) > 10:
    #     splitSeqList = [souLangText[i:i + int(len(souLangText) / 5)] for i in
    #                     range(0, len(souLangText), int(len(souLangText) / 5))]
    # for idx, eseqs in enumerate(splitSeqList):  # 文本过长了，批次送文本，保证不改变指令的情况使用
    #     tp3 = part3(eseqs)  # 第三轮对话
    ...
    #         allseqClsDict.update(eval(res_part3))
    #     except:
    #         continue
    # if allseqClsDict:
    #     logging.info(f"【过程】判定step-3机翻文本类型过程结束，得到如下结果：\n"
    #                  f"完成分类的数据信息：{allseqClsDict}\n"
    #                  f"--------------------")
    # else:
    #     logging.info(f'【过程】Step-3数据类别判定过程中出错，程序退出；\n\n')
    #     return '0'
    # res_part3 = str(allseqClsDict)
    # hisTalks['message'].append({'content': res_part3, 'role': 'assistant'})
    # pass
    # mtList, gpttList, splitSeqList = [], [], []
    ...
    #     else:
    #         gpttList.append(k)
    # logging.info(f'【过程】Step-4：如下文本将采用Baidu/Google/DeepL等翻译工具处理：\n'
    #              f'{mtList}')
        ...
        try:
            for future in concurrent.futures.as_completed(to_do):  # 并发执行
                ...
                if len(_res_part4_) == 2:
                    ...
                    logging. \
                        info(f'【过程】在解析 step-4 Threads Finale result 时出错，这是模型Prompt的问题。出错信息已记录 \n')
        except:
